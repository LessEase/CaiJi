...
if __name__ == "__main__":
    ...
    clfs = [GradientBoostingClassifier(loss='deviance', learning_rate=0.05, n_estimators=300, max_depth=28, min_samples_split=3, min_samples_leaf=2, subsample=1.0, max_features=0.7, max_leaf_nodes=None),
RandomForestClassifier(n_estimators=300, criterion='gini', max_depth=25, min_samples_split=2, min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_features=0.6, max_leaf_nodes=None, bootstrap=True, oob_score=False, n_jobs=4, random_state=None, verbose=0, warm_start=False, class_weight=None)
]
    for i in range(len(clfs)):
        print "training model ", i+1
        if not os.path.exists("models/model"+str(i)):
            os.makedirs("models/model"+str(i))
        clfs[i].fit(trainX, trainY)
        joblib.dump(clfs[i], "models/model"+str(i))
